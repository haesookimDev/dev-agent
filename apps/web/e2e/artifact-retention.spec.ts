...
for (const [locale, title, expired, open, original] of [
  ...
  for (const kind of ["artifact-retention", "scheduled-retention"]) {
    test(`${locale} actual ${kind} CLI-expired evidence keeps metadata without dead actions`, async ({ page, request }, testInfo) => {
      const works = await (await request.get("http://127.0.0.1:18100/api/work-items")).json();
      const work = works.find((row: { repository: string }) => row.repository === `demo/${kind}`);
