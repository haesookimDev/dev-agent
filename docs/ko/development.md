...
개발은 기능별 작업 브랜치에서 수행하고, 논리적 단위마다 관련 테스트를 실행한 뒤 한국어 메시지로 커밋합니다. 자동 테스트와 실제 사용 검증이 통과하면 증거를 포함한 PR을 만들고 최신 커밋의 CI와 리뷰 상태를 확인합니다. 사용자가 머지를 위임한 작업은 에이전트가 Merge Commit으로 머지하고 `main`을 Fast-forward 동기화한 뒤 다음 기능 브랜치를 생성합니다. Draft 생성은 작업 완료가 아닙니다.

### 기능별 PR을 끝내는 반복 절차

1. 구현 전에 PR의 한 가지 목적, 선행 의존성, 정상·실패 경로와 실제 구동 검증을 정합니다. 하나의 장기 통합 브랜치에 여러 기능을 계속 쌓지 않습니다.
2. 해당 PR의 테스트·실제 사용·CI 실패와 리뷰 의견은 수정하고 영향을 받는 검증을 다시 실행합니다. 해결 가능한 문제가 남은 채 새 Draft를 계속 만들지 않습니다.
3. 기능 자체의 필수 구현·검증이 남았을 때만 Draft로 유지합니다. 해당 조건을 충족하면 Ready로 전환하고 정확한 최신 Head의 필수 CI·리뷰를 확인한 뒤 위임된 머지를 수행합니다.
4. 외부 입력 없이는 진행할 수 없으면 차단 사유, 이미 시도한 검증, 필요한 입력과 재개 조건을 PR에 기록하고 사용자에게 보고합니다. 이때만 독립적인 다른 작업을 별도 브랜치에서 진행할 수 있습니다. 검증 생략·보호 우회로 Draft를 없애지는 않습니다.
5. 선행·후속 PR은 의존성 순서대로 마무리합니다. 선행 병합 뒤 후속 PR의 Base, 남은 Diff·Commit, 필요한 재검증과 최신 CI를 확인합니다. Squash·커밋 재작성·일괄 머지로 기능 경계를 없애지 않습니다.

**기능 PR 병합과 전체 MVP 출시 승인은 별개입니다.** 예를 들어 Worker의 정리·복구 PR은 해당 VM/API 경로를 검증하고, 별도 GUI 기능의 미완료만으로 Draft에 묶어 두지 않습니다. 반대로 그 PR 자체가 변경한 GUI·권한·복구 경로의 검증을 후속 작업으로 미룰 수는 없습니다. 전체 MVP의 미완료 기준과 고정 완료율은 [진행 기록](mvp-progress.md)에 별도로 남깁니다.
...
[시간 예산 연장 UI](budget-approval-ui.md)는 확인 시점의 버전을 고정하며 실제 API의 승인·재소진·충돌과 응답 유실을 검증합니다. `budget.spec.ts`는 임시 Scoped Worker로 작업을 준비하고 자신의 임대를 해제한 뒤 공통 검증을 따릅니다. 테스트의 개발 인증이나 403 응답 Fixture를 실제 OIDC·VM 검증으로 보고하지 않습니다. 정상 확인 창 스크린샷도 기존 CI 검증 자료에 포함합니다.
...
현재 `CI` Workflow의 필수 검사는 `Python`, `Go`, `Web`입니다. API 전체 수집을 `API (1/2)`·`API (2/2)`로 병렬 실행하고, 두 묶음이 성공해야 `Python`이 Runner·Ruff와 기존 PostgreSQL 17 Migration·잠금·격리·감사·보존·경쟁·관측·SSE·복원 검사를 통과할 수 있습니다. Go는 Worker·Gateway 테스트와 vet를 실행합니다. Web은 테스트·타입 검사·ESLint·운영 빌드·Chromium E2E를 실행합니다. 브라우저 바이너리를 캐시하고 보고서와 실패 증거를 `browser-evidence` Artifact로 7일간 보존합니다. 다섯 Job의 제한은 각각 8분이며, PR에서는 이전 실행을 취소합니다. [API 분할과 실패 차단 검증](ci-partitions.md)에 따라 두 묶음과 후속 DB 검사를 모두 확인하세요. 경로 기반 생략이나 다중 Version Matrix는 사용하지 않습니다.
...
[화면 이동](navigation-scroll.md)을 변경할 때 전역 Scroll 설정과 Next.js Layout 계약, 모션 감소, Anchor·건너뛰기 Focus, 뒤로 가기를 함께 검증합니다. `navigation-scroll.spec.ts`는 기존 Web CI에서 실제 서비스·Chromium으로 실행하고 스크린샷을 기존 검증 자료에 포함합니다. 경고를 숨기거나 확인하지 못한 네이티브 입력을 성공으로 보고하지 않습니다.
