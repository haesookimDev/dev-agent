...
제품 기준 코드: `main`의 `fb1f57eab91a9feef031e236d289c8d74ec03942` ([PR #57](https://github.com/haesookimDev/dev-agent/pull/57) 병합). Mac 로컬 개발 호스트 검증 기반은 병합됐으며 정확한 Head `ab9378a`의 [CI](https://github.com/haesookimDev/dev-agent/actions/runs/34447949823)는 5개 Job 모두 통과했습니다. 미병합 `feat/golden-image-arm64`의 기능 기준은 `cea2e53`, 상세 실행 기록은 `2385150`입니다. 기존 후보의 Recipe는 `34ae353`이며 최신 `cea2e53` 재빌드는 9월 11일 10:22 KST에 시작했습니다. GitHub `main`과 Draft #55의 Head는 9월 11일 재확인했으며 ARM64 후속은 미Push·새 Head CI 미실행 상태입니다. 커밋된 코드와 비공개 초안을 구분한 기록이며 Draft 브랜치나 계획은 완료로 세지 않습니다.
...
후속 ARM64 통합은 기존 후보의 Boot 실패를 진단해 QGA 종료 응답 수집(`fabc600`), Browser 하위 프로세스 정리(`7b46236`), 전용 CDP 검사(`2c52b12`), 정확한 AppArmor 정책/파일 무결성 검사(`2fa0cdd`)를 구현했습니다. 실제 재빌드에서 발견한 Guest 도구 전송 누락은 `cea2e53`으로 수정했습니다. 최신 **Linux 170개 전부 통과**(4.493초), **Mac 169개 통과/플랫폼 Skip 1개**(9.107초), `make lint`, Packer 형식/구문 검사가 통과했습니다. 최종 CDP Helper는 실제 Overlay에서 Viewer 연결 없이 12.01초에 DOM·정상 종료·cgroup 정리를 통과했고, 전역 AppArmor 제한과 기본 프로필의 Capability 거부도 확인했습니다. 정책 Helper는 실제 316개 파일/디렉터리와 기존 정책을 재검증했습니다. 이는 **새 후보의 깨끗한 두 Boot·데스크톱 입력·제품 Console 인수 검증은 아닙니다.** 새 AppArmor 고정값을 포함한 입력으로 재빌드 중이며 [Golden Image 기록](golden-image-inputs.md)에 커밋·실행·실패·남은 조건을 구분합니다.
