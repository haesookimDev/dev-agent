...
4. [Draft PR #58](https://github.com/haesookimDev/dev-agent/pull/58), Head `2780f4d3d3f9ba6dc6647293c61fef64aff88403`: 영속 Run 기록·정리 전 자원 반환 금지, 실제 빈 ARM KVM VM의 종료/취소 두 사례에서 종료→정의 해제→파일 제거→모의 API 확인을 검증했습니다. [고정 SHA 증거](https://github.com/haesookimDev/dev-agent/blob/2780f4d3d3f9ba6dc6647293c61fef64aff88403/docs/ko/worker-lifecycle.md), [CI 5개 통과](https://github.com/haesookimDev/dev-agent/actions/runs/34560620314)(첫 실행 약 5분 20초). 실제 Golden Image/Runner를 통한 전체 실행, 전용 ARM Firmware·작업별 네트워크·시간 예산, 이번 API와의 재시작 연동은 남아 있습니다. 빈 VM 두 개의 순차 검사를 동시 두 작업 Acceptance로 계산하지 않습니다.
5. 다음은 Worker가 Claim 임대 ID를 영속 Run ID에 연결하고, 정리 완료 뒤에만 이번 API로 재확인하는 통합입니다. 실행 중 작업의 복구 정책과 Claim 응답 유실도 별도 검증합니다. [Draft PR #21](https://github.com/haesookimDev/dev-agent/pull/21)의 Preview는 아직 `main`에 없습니다. 작업별 네트워크·TLS·Console·동시 두 작업이 입증되기 전에는 완료로 계산하거나 강제 병합하지 않습니다. 기존 운영 Host나 주변 자격증명을 임의로 찾아 사용하지 않습니다.
