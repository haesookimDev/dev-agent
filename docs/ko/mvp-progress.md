...
5. PR #58의 최신 Head `0274718914ccf193fc7caf51ce561b08a242a61c`는 [CI 5개](https://github.com/haesookimDev/dev-agent/actions/runs/34566826882)가 첫 실행 6분 1초에 모두 통과했고 미해결 리뷰는 없지만 Draft입니다. 후속 [ARM Firmware 소유권](worker-firmware.md)의 소스 `dafc0dab677756887b2707871e83deb3ab1748da`에서 실제 실행기 생성·UEFI·정리 두 사례가 48.90초에 통과했습니다. amd64의 기존 부팅 선택을 보존하며, Fixture가 NIC/Graphics를 제거하고 빈 디스크·모의 API를 사용한 한계를 기록했습니다. 이 미병합 증거도 전체 Executor 또는 릴리즈 단계 완료로 계산하지 않습니다.
6. ARM Firmware [Draft PR #60](https://github.com/haesookimDev/dev-agent/pull/60)의 Head `31793bcb9d91ac8d93b7b7c1dde7109290670d0e`도 [CI 5개](https://github.com/haesookimDev/dev-agent/actions/runs/34567965046)가 첫 실행 5분 17초에 통과했습니다. 별도 [VM 명령 제한](worker-command-limits.md)의 소스 `ae81537ae8b6b155614ba123a8f4df02dbdbc253`에서는 명령별 45초·취소 시 자식 종료와 실제 VM 생성 후 명령 취소를 검증했습니다(실제 3사례/70.64초). 이는 전체 작업의 시간 예산 강제와 구분하며 모두 아직 미병합입니다.
7. 명령 제한 [Draft PR #61](https://github.com/haesookimDev/dev-agent/pull/61), Head `9d055382e7d1bcd428b2bbccea7daea66c56b88d`의 [CI 5개](https://github.com/haesookimDev/dev-agent/actions/runs/34568940739)가 첫 실행 5분 45초에 통과했습니다. 후속 [작업별 네트워크 기반](worker-network-foundation.md)의 소스 `50ada438156ea596cb07d317911bf12f715e194b`에는 충돌 거부 주소 할당·Network/전면 차단 Filter XML과 실제 Linux의 libvirt 스키마 검사가 추가됐습니다. 아직 Executor에 연결하지 않았고 실제 패킷 검증도 아니므로 격리 완료나 릴리즈 완료율에 가산하지 않습니다.
8. 후속 [네트워크 수명주기](worker-network-lifecycle.md) 최신 소스 `c53fea9ecf6d48d2473550ff0e343311845bfa46`에는 Schema 3 영속 예약 직렬화, 현재 Host 충돌 재고 수집과 산출물 삭제·확인 이전의 소유 자원 생성/정리 검증이 추가됐습니다. 실제 정의·활성 Network 정리가 3.17초에, 이후 정상 생성·활성화 직후 취소와 별도 프로세스 복구가 4.61초에 통과했습니다. 이 검사는 Guest NIC·실제 API 반환·패킷 격리를 검증하지 않습니다. Worker 테스트·Race·정적 검사가 통과했습니다. 이 기록은 PR 제출 이전이며 이후 정확한 Head CI는 PR에 기록합니다. 릴리즈 완료율에는 가산하지 않습니다.
9. 네트워크 수명주기 [Draft PR #62](https://github.com/haesookimDev/dev-agent/pull/62)의 Head `5c79ea634286fa88287c0a02961c24171d21c0e9`는 [CI 5개](https://github.com/haesookimDev/dev-agent/actions/runs/34574335659)가 첫 실행 5분 34초에 통과했습니다. 후속 소스 `071f95f7c96ec16bbf965cf4fcb3b6048122923f`에서 [실제 Golden Image NIC의 전면 패킷 격리](worker-network-packets.md)를 검증했습니다. 게스트·Host 방향 각각 12개 프레임의 실제 DROP 카운터 증가, 원본 이미지 소유자·해시·ACL 보존, 소유 VM/Network 정리가 109.96초에 통과했습니다. 원본 이미지 소유권 변경을 발견해 테스트용 읽기 전용 접근을 수정하고 재검증한 기록도 포함합니다. 운영 Executor·Runner·허용 송신 정책·동시 두 VM의 검증은 아니며 미병합 부분 증거이므로 완료율은 1/7(14.3%) 그대로입니다.
