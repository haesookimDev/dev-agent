...
8. 후속 [네트워크 수명주기](worker-network-lifecycle.md) 소스 `26476bebcc385cedbbc8259ec239803680d68246`에는 Schema 3 영속 예약 직렬화와 산출물 삭제·확인 이전의 정확한 Network/Filter/Bridge 정리가 추가됐습니다. 실제 정의·활성 Network 두 경우가 3.17초에 통과했고 앞서 남은 Fixture는 별도 복구 프로세스로 정리했습니다. 이 검사는 Guest NIC·실제 API 반환·패킷 격리를 검증하지 않습니다. Worker 테스트·Race·정적 검사가 통과했지만 미제출 브랜치의 최종 Head GitHub CI 결과는 없으며 릴리즈 완료율에 가산하지 않습니다.
9. 다음은 현재 Host 충돌 재고를 수집하고 안전한 생성·Host 경계·송신 정책을 연결한 뒤 전체 Executor의 Golden Image/Runner 경로와 전체 VM 시간 예산을 검증하는 단계입니다. 실행 중 작업의 복구 정책과 Claim 응답 유실도 별도 검증합니다. [Draft PR #21](https://github.com/haesookimDev/dev-agent/pull/21)의 Preview는 아직 `main`에 없습니다. 작업별 네트워크·TLS·Console·동시 두 작업이 입증되기 전에는 완료로 계산하거나 강제 병합하지 않습니다. 기존 운영 Host나 주변 자격증명을 임의로 찾아 사용하지 않습니다.
