...
- [Runner 이벤트 전송 경계](runner-event-redaction.md)와 [API 이벤트 수신 경계](api-event-redaction.md)는 알려진 평문 임대와 명시적 자격증명 필드를 가리고 실제 명령·직접 HTTP·SSE·DB·브라우저로 검증합니다. 범용 Secret Scan이나 모든 API·Artifact·Crash Dump·cloud-init 보호의 완료를 뜻하지 않습니다.
