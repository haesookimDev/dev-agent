...
운영자가 보존 기간과 저장소를 확인한 뒤 제어 호스트에서 실행하는 제한된 수동 CLI입니다. 후속 [예약 Worker](scheduled-retention.md)는 진행 위치를 DB에 유지하며 별도 프로세스로 실행합니다. 어느 쪽도 API 정리 엔드포인트나 기본 보존 기간을 추가하지 않습니다. 수동 CLI는 기존 `DATABASE_URL`과 `ARTIFACT_ROOT`를 사용하며 새로운 환경변수나 의존성은 없습니다. 대상 파일을 unlink하므로 승인된 보존 정책 없이 운영에 적용하지 마세요.

1. DB·파일 복구 지점을 [백업 절차](artifact-backup.md)에 따라 확보합니다.
2. 현재 코드의 Migration Head `20260909_0011`까지 적용하고 **모든 API 인스턴스와 정리·백업 CLI를 함께 업그레이드**합니다. 만료 기록은 0010, 예약 진행 위치는 0011에서 추가되었습니다. 이전 API가 실행 중이면 만료 의도를 해석하지 못할 수 있으므로 정리를 시작하지 않습니다.
