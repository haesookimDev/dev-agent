...
[Work-scoped ordinary artifact writes/reads](artifact-isolation.md), [executable-document rejection with download content revalidation](artifact-content.md), [international filenames/retained-header safety](artifact-filenames.md), and [HTTP storage prevention with fresh recovery/access responses](artifact-cache.md) were added (2026-09-06). Retained out-of-scope keys, descendant links and unsupported MIME are not served. Artifact-viewer contrast/error guidance and physical file backup/restore remain separate follow-up work.
