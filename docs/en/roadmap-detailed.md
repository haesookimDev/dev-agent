...
Added [ordinary-artifact scheduled retention](scheduled-retention.md) with bounded sweeps, durable progress, restart/concurrent-write protection and an explicit service template (2026-09-09). Scheduled cleanup for other data types and actual operational deployment are not included in this completion scope.

[Work-scoped ordinary artifact writes/reads](artifact-isolation.md), [executable-document rejection with download content revalidation](artifact-content.md), [international filenames/retained-header safety](artifact-filenames.md), [HTTP storage prevention with fresh recovery/access responses](artifact-cache.md), and [read-only preview, contrast, error recovery and keyboard navigation](artifact-preview.md) were added (2026-09-06). Retained out-of-scope keys, descendant links and unsupported MIME are not served. [DB-bound local ordinary-file backup/new-root restore](artifact-backup.md) and [explicit active-work-safe retention CLI, expiration UI and backup V2](artifact-retention.md) are also implemented. Other data retention policies, scheduled janitors and external object-store recovery remain follow-up work.

Status: [administrator cancellation of unassigned queued work with auditing](work-cancellation.md), [PostgreSQL backup/new-database restoration with data/permission/audit verification](postgres-restore.md), and [delivery-patch byte verification with fixed-copy application](delivery-integrity.md) are implemented. Active-run cancellation with verified VM termination/cleanup, retry, forced lease release, non-ordinary-file retention policies/scheduled janitors, external object-store recovery, and actual operational recovery verification remain.
