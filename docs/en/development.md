...
Seven [scheduled-retention](scheduled-retention.md) cases in `test_retention_job_postgres.py` join the same PostgreSQL step (38 total). `test_retention_worker.py` checks actual separate processes, SIGTERM and restart; `test_retention_service.py` checks dry-run defaults, required policy and Linux systemd syntax. The syntax check skips on non-Linux hosts such as macOS; Linux CI must have `systemd-analyze`. Web retains manual-CLI checks and adds two scheduled-Worker expiration journeys. There is no new job, service matrix or timeout increase. Scheduled dry-run writes only progress metadata, unlike the fully read-only manual dry-run.
...
The current `CI` workflow requires `Python`, `Go` and `Web`. The complete API collection runs in parallel as `API (1/2)` and `API (2/2)`; both must succeed before `Python` can pass Runner/Ruff and the existing PostgreSQL 17 migration, locking, isolation, audit, retention, race, observation, SSE and restore checks. Go runs Worker/Gateway tests and vet. Web runs tests, type checking, ESLint, the production build and Chromium E2E. Browser binaries are cached; reports and failure evidence are retained in the `browser-evidence` artifact for seven days. All five jobs have eight-minute timeouts; superseded PR runs are cancelled. Follow [API partition and failure-gate verification](ci-partitions.md) to inspect both partitions and subsequent database checks. There is no path-based skipping or multiple-version matrix.
...
[Execution-phase/delivery observation](execution-monitoring.md) uses the same PostgreSQL aggregate checks and actual HTTP query-failure/recovery regression in `test_execution_health_http.py`. `make test-monitoring` checks every state in all four families, invalid/duplicate/missing data, phase alert timing and recovery. Do not interpret human waits as stalls or metadata age as evidence of actual VM progress. Existing CI jobs and timeouts remain unchanged.

When changing [route scrolling](navigation-scroll.md), verify global scrolling and the Next.js layout contract together with reduced motion, anchors/skip-link focus and browser history. `navigation-scroll.spec.ts` runs actual services/Chromium in existing Web CI and includes screenshots in its existing evidence artifact. Do not hide warnings or report unverified native input as successful.
