...
This strengthens a specific Runner egress boundary; it is not a general secret scanner or protection against an adversarial VM. Unknown tokens in free text, encoded/fragmented/image secrets, other exceptions' earlier truncation/local tracebacks/crash dumps, artifact/delivery-bundle bytes and cloud-init are not comprehensively covered. A separate follow-up strengthens a bounded [API event ingress boundary](api-event-redaction.md) for direct writes bypassing the Runner. Previously retained events are not rewritten. SEC-001, actual VM isolation and the full MVP remain incomplete.
