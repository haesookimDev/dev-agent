...
Orca's dark-mode native text/JSON documents displayed white text on a white background. Light-mode comparison showed black, readable content. This is distinct from captures where the target tab was not revealed; native coordinate input was rejected by the focus guard and not retried. [Filename encoding](artifact-filenames.md) was verified in a subsequent change. Artifact-viewer contrast/error guidance and the existing cached-410 issue remain separate follow-up PRs. No product UI code changes in this PR.
