...
Verified [actual `from`/`to` retention](transition-metadata.md) in history generated by the common state transition function. Arbitrary-event provenance and caller-specific transition authority remain separate concerns.
