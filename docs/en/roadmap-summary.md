...
2. **Partially complete:** OIDC authentication, organization/repository authorization, append-only [feedback](feedback-audit.md) and [console/approval](control-action-audit.md) audits, and [session/work-scoped HTTP Preview grants](preview-access.md) are implemented. Cancellation/delivery audits remain. Actual public DNS, enterprise IdP and VM-network Preview validation remain separate release requirements.
