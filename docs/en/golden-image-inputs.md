...
[`prepare.py`](../../infra/images/prepare.py) only verifies inputs and prepares separate copies. [`build.py`](../../infra/images/build.py) and the [Packer template](../../infra/images/ubuntu.pkr.hcl) invoke [`guest.py`](../../infra/images/guest.py) installation/sealing followed automatically by two [`boot.py`](../../infra/images/boot.py) boot checks on an approved dedicated KVM host. **Actual candidate build, sealing, two cold boots and limited desktop/browser input were verified on ARM64 KVM inside the Mac. Actual amd64 execution, full acceptance, release gates and IMG-001 remain incomplete.** The chronological record below distinguishes failures, fixes and rechecks. Existing Worker base-image configuration/execution is unchanged and no automatic rollout occurs.
...
ARM64 uses `virt`, KVM, host CPU/GIC, virtio GPU/keyboard/absolute-position tablet and SCSI seed CD. The first probe starts with a pinned blank VARS copy, not builder-mutated NVRAM; both cold boots retain the same private VARS and disk overlay. CODE is read-only, and neither host firmware nor the original candidate is a writable VM target. amd64 retains q35. Actual Worker ARM64 lifecycle and console integration require separate acceptance; this builder does not complete them.
...
To migrate inputs, resolve an exact `apparmor` version from the pinned snapshot, prepare inputs under a new `image_version`, and **rebuild a new candidate**. Do not edit existing manifests/completion receipts or candidates in place. Adding the verified ARM snapshot's `apparmor=4.0.1really4.0.1-0ubuntu0.24.04.7` produced `20260911-arm64-candidate.1`: 12 files, 17 APT pins, manifest SHA-256 `fa46d223c85df43cbf4781d14262e3c4eb1dc69686668bb302b82dfb7f4d8736`. The standard rebuild was underway at this point; the next section records its outcome. Never promote a failed candidate; rollback must use a separately approved prior image, not the preserved unverified candidate.

### ARM64 standard build, two boots and input-device fix, September 11

On the Mac M4 Pro's dedicated Lima/Ubuntu ARM64 KVM host, `run-06` from exact Git archive `cea2e53` completed installation, sealing and both automatic cold boots with exit 0. All eight guest checks passed in both boots, with the same machine-ID hash, distinct boot-ID hashes, clean QEMU exits and unchanged candidate hash. The candidate is **5,074,386,944 bytes**, SHA-256 `b2de98bf1725ede0319b2e50c74d4dc9de3499846cdeb3b7ca880b0bd22963a6`, recipe SHA-256 `ee18916794636455c847f15d461e212bf71bbd2c717a27ff31ae17b24251e295`. `boot_smoke_passed_unreleased` is not release approval; `release_eligible=false` remains.

Subsequent actual GUI testing exposed missing input devices that headless checks had not detected. The previous VM's Linux input inventory contained only Power Button, and `Alt+F2` did nothing. `70fd562` adds `virtio-keyboard-pci` and `virtio-tablet-pci` to both Packer and boot ARM64 device lists, without host input passthrough or amd64 changes. Two regressions failed before the fix; all 48 relevant tests passed afterward (1.673s), alongside Packer formatting/syntax and `make lint`.

The actual VM with the corrected device configuration exposed QEMU Virtio Keyboard/Tablet. Computer Use operated a dedicated Chrome window on the Mac: `Alt+F2` opened Xfce Application Finder, Chrome for Testing `153.0.8010.36` launched, a local verification page loaded, its empty required field showed a validation error, and the guest visibly received `Mac ARM64` with correct capitalization. The HTML fixture is an unprivileged file in a separate overlay, not a modification to the source image or product dashboard.

| Before: visible desktop without input devices | After: actual guest browser receives input |
| --- | --- |
| ![Xfce before the input-device fix](../assets/golden-image-arm64/before-input-devices.png) | ![Browser showing Mac ARM64 after the fix](../assets/golden-image-arm64/after-keyboard-input.png) |

At `70fd562`, **all 171 image tests passed on Linux** (4.551s). During the Mac full suite, the EOF-ordering test's initial `Browser.getVersion` request exceeded its 0.5-second budget before the EOF stage; separate commit `8217967` uses the same two-second limit as other success paths. Missing EOF still fails, and the real browser's 24-second deadline is unchanged. Final checks passed **170/one platform skip on Mac** (6.320s), `make lint`, and the 12 affected Linux regressions (1.487s). The Linux host lacks `make`, so `python3 -m unittest discover -s infra/images/tests -v` ran the same image suite. No new dependency, CI job or timeout increase was added.

Installed image sources have not changed since `cea2e53`. The follow-up only changes host QEMU input-device attachment and regressions; reprobes preserve separate `source_recipe_sha256` and `probe_tooling_sha256` records. Running the standard standalone boot CLI at `8217967` with a fresh candidate copy, seed, overlay and pinned blank firmware copy produced `boot-final-06`: **all eight checks, clean shutdown and unchanged source hash passed for both boots**. The [original build's boot receipt](../assets/golden-image-arm64/build-two-boots.json) and [final-tooling reprobe receipt](../assets/golden-image-arm64/final-tooling-two-boots.json) are actual JSON results without raw IDs, environment or credentials. The existing image is not claimed to have been reinstalled with the new Packer template.

The private GUI viewer connects only to loopback VNC on a VM with no NIC; it **does not establish product Console authentication/input ownership or per-work network acceptance**. Long-text key-event delivery failed in this verification path; the unresolved failure is not a passing result. `arm-build-06.log`, `boot-final-06.log`, `desktop-06-fixed-inputs.log`, `arm-input-devices-{red,targeted,linux,mac-final}.log` and `browser-eof-linux-final.log` remain in the dedicated local evidence directory.

The additional `desktop-06-final-gui.log` run failed at 300 seconds while GUI interaction overlapped health checking. It is neither counted as a pass nor combined with the standard two-boot evidence. The exact bottleneck is not established; subsequent GUI-only diagnostics separate interaction from automatic health checks. Only the Mac viewer was relaunched with a separate profile and [official tooling background-throttling flags](https://github.com/GoogleChrome/chrome-launcher/blob/main/docs/chrome-flags-for-tools.md); guest browser, sandbox and product timeouts were unchanged. Actual-use responsiveness and product-path integration remain acceptance requirements.

The subsequent `desktop-06-gui-only.log` run verified Xfce's launcher, browser relaunch and cancellation of an initial dialog through the local RFB pointer with the same input devices. The `/tmp` fixture was absent after reboot; recreating it unprivileged loaded the page, but the 420-second GUI-only diagnostic window ended **before form submission could be reverified**. This neither increases product boot/browser deadlines nor establishes automatic health acceptance. The owned VM stopped and the original candidate hash remained unchanged.

Original candidates and failure logs were preserved. To recover space, only the single derived copy `run-04/boot-check/candidate.qcow2` from an older failed run was removed after ownership, full-hash and non-use checks. It can be restored at the exact original path from the same-hash `run-04/image/kelpie.qcow2`. No user files or production images were deleted. Actual amd64 build/boot/GUI, signing, SBOM, vulnerability/provenance/canary gates, comprehensive credential non-disclosure and real Worker lifecycle acceptance remain outstanding; MVP stays **1/7 = 14.3%**.
