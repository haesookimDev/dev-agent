...
The prerequisite ownership changes in PR #58 merged into `main` at `90af0a4e7e70a5795e95dc85e95add01a45e478d`. PR #60 now targets `main` so its comparison contains only firmware changes. Apply only after verified shutdown/draining and retain records/capacity on unconfirmed cleanup. Roll back to the prior binary after verified shutdown; do not start new ARM work with an old Executor that lacks the owned ARM firmware path. Never bypass recovery by deleting records or manually writing released state. The evidence above verifies this feature's actual creation, firmware and cleanup paths; final-head CI, review and merge status are recorded in [PR #60](https://github.com/haesookimDev/dev-agent/pull/60). Whole Golden Image/Runner, networking and GUI conditions belong to separate features and MVP release criteria, not this PR's Draft rationale. The fixed MVP measure remains 1/7 (14.3%). Reinspection on 2026-09-13 confirmed documentation/log-only changes since final source and the original evidence hashes; it does not claim a new actual VM execution.
