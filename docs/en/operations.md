...
Online migrations of development SQLite databases use `BEGIN IMMEDIATE` to place the entire revision chain in one transaction. A later revision's safety-gate failure rolls back earlier DDL, row changes and revision numbers together. Other writers may wait or receive lock errors during migration; stop API/Worker writes for maintenance and resume after success. Never bypass a refused downgrade by stamping the database or deleting audit/expiration evidence. This fix adds no revision, environment variable or API format; reverting the migration runner can reintroduce partial changes, so prefer correcting the cause and retrying.
...
When the OTLP endpoint is empty, application spans are not exported, while Prometheus metrics and structured logs remain available. [Baseline alerts and runbooks](monitoring-alerts.md) cover failed/missing scrapes, delayed startup recovery, and observed delivery failures. [Continuous observation alerts](runtime-monitoring.md) monitor lost Worker heartbeats, expired active leases, and long-queued work without treating failed, stale, or missing observations as healthy. [Execution/DeliveryJob metadata-age alerts](execution-monitoring.md) also cover phase thresholds and missing/duplicate extended metrics. External object-store, SCM and delivery-worker readiness, progress-based latency signals and an integrated operations dashboard remain follow-up OBS-001 scope.
...
`libvirt` requires separate `KELPIE_GUEST_CONTROL_URL` and `KELPIE_GUEST_CONTROL_IPV4` values, not the host endpoint. Optional Worker input `KELPIE_GUEST_CONTROL_CA_FILE` supplies generated guest `KELPIE_CONTROL_CA_FILE`, scoped to ControlClient. See [guest control bootstrap](guest-control-bootstrap.md) for the exclusive API listener, address pool, seed/bootstrap, drained rollout and Schema 4 rollback contract. Default public PKI and TLS checks from [Runner-specific CA trust](runner-control-tls.md) remain unchanged.
