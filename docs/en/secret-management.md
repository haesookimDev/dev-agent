...
- [Delivery-failure diagnostics](delivery-failure-safety.md) store fixed stages and error codes in the database, events, and traces instead of raw Git/upstream exceptions. That guide records real Git failure, approval/browser verification, and operational limitations.
- [Runner event egress](runner-event-redaction.md) and [API event ingress](api-event-redaction.md) redact the known plaintext lease and explicit credential fields, verified with actual commands/direct HTTP/SSE/database/browser use. This does not complete general secret scanning or protection for every API, artifacts, crash dumps and cloud-init.
- OIDC/Slack network consumer tests use mock providers. [Worker control-plane quarantine](worker-quarantine.md) is implemented, but actual host/VM/network/existing-connection containment, external accounts, complete event/artifact/crash-dump/cloud-init scanning, and production secret policy remain unfinished. SEC-001 as a whole is not complete.
