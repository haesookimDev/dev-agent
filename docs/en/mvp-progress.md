...
Product baseline: `main` at `fb1f57eab91a9feef031e236d289c8d74ec03942` (merged [PR #57](https://github.com/haesookimDev/dev-agent/pull/57)). Unmerged work is recorded separately: ARM64 image PR #55 at `b865954101a4becd37d23f4aadf39a9d9795585f` and actual verified source `74a8edf35e6d3e3d4ae1b6aefee729473112ea47` on `feat/worker-owned-lifecycle`. This snapshot compares code, tests and actual runtime evidence; draft branches and plans do not count as complete.
...
| 5. Golden image and real libvirt execution | **Incomplete.** Draft image PR #55 has actual ARM64 build/two-boot evidence. Current [Worker lifecycle](worker-lifecycle.md) verifies persistent ownership, cancellation cleanup, ACLs and capacity returned after cleaning two real blank VMs, but remains unmerged. | Actual amd64 image checks and remaining GUI gates; complete executor Golden Image/Runner integration and normal OS shutdown; API lease reconciliation/restart/orphan recovery, per-work networks and physical time-budget enforcement. |
...
At PR #57, local `make test` with dedicated PostgreSQL passed API 1270/1 skip (systemd parser runs on Linux CI), Runner 45, Web 125, Go and Lab 18. Its exact-head `ab9378c` CI passed in 5m20s. That historical result is not full verification of the current Worker changes. This Worker work passed `make test-worker`, race tests, `make lint`, relevant Linux regressions, two real libvirt cases in 43.62 seconds and recovery of earlier failed artifacts. Follow the [detailed evidence and limits](worker-lifecycle.md); record exact latest-head CI, review and merge status in the corresponding PR.
...
3. On the approved ARM64 Linux/KVM host inside the Mac, [Draft PR #55](https://github.com/haesookimDev/dev-agent/pull/55) verified image creation, two boots and actual keyboard input. [Immutable image evidence](https://github.com/haesookimDev/dev-agent/blob/b865954101a4becd37d23f4aadf39a9d9795585f/docs/en/golden-image-inputs.md) and [latest-head CI](https://github.com/haesookimDev/dev-agent/actions/runs/34556143436) record all five checks passing first attempt in 7m43s. Keep it Draft until actual amd64 execution, remaining GUI and complete execution acceptance. Current Worker blank-VM force-cleanup cases are sequential, not substitutes for normal OS/Runner, restart API reconciliation, per-work networking, TLS, Console or concurrent two-work acceptance. Do not use ambient production hosts or credentials.
